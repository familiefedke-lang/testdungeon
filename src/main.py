...
def main() -> None:
    ...
    spritesheet = Spritesheet(ATLAS_PNG, tile_size=TILE, tiles_per_row=8)
    animation_db = AnimationDB(ATLAS_JSON)
    font = pygame.font.SysFont("monospace", 14)

    renderer = Renderer(screen, spritesheet, animation_db, font)
...
def _generate_placeholder_atlas(path: str) -> None:
    ...
    Layout (8 tiles per row, 32×32 each):
      ...
      Index 11 → goblin hurt S (orange-red)
      Index 12 → goblin dead S (brown)
    """
    import pygame
    ...
    for idx, color in enumerate(palette):
        ...
        # Draw a small centered shape so tiles look distinct from each other

...
def generate_floor(
    ...
    # ------------------------------------------------------------------
    item_rooms = rooms[1:] if len(rooms) > 1 else rooms[:]
    ...
    if not item_candidates:
        item_candidates = [
            ...
            if game_map.get_tile(x, y) == TileType.FLOOR and (x, y) != (start_x, start_y)
        ]

    rng.shuffle(item_candidates)
    if item_candidates:
        hx, hy = item_candidates.pop()
        game_map.entities.append(Item(hx, hy, item_key="helmet", name="helmet"))
    if item_candidates:
        ax, ay = item_candidates.pop()

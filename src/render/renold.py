...
class Renderer:
    ...
    def __init__(
        ...
        items_atlas: "ItemsAtlas",
        font: pygame.font.Font,
    ) -> None:
        self.screen = screen

        # Main (tiles + actors) sheet/db
        self.spritesheet = spritesheet
        self.animation_db = animation_db

        # Items sheet/db
        self.items_spritesheet = items_spritesheet
        self.items_atlas = items_atlas
    ...
    def draw_item(self, item: "Item") -> None:
        tile_index = self.items_atlas.get_item_index(item.item_key, default=0)
        surface = self.items_spritesheet.get_tile_surface(tile_index)
        self.screen.blit(surface, (item.x * TILE, item.y * TILE))
